...
= \lim_{h \to 0} \frac{I(x+h,y) - I(x,y)}{h}  
$$
...
$$  
I_x =  
(-1)p_{11} + 0p_{12} + 1p_{13}

+ (-2)p_{21} + 0p_{22} + 2p_{23}
  
+ (-1)p_{31} + 0p_{32} + 1p_{33}  
$$
展开逻辑：
1. 中间列全零，表示不参与左右对比。
...
$$
结果 $I_x$ 就表示**这个 $3×3$ 小区域在 $x$ 方向上的变化强度。** 在这个例子里，它就表示 **$p_{22}$ 位置在 $x$ 方向上的近似偏导。**

现在，对整幅灰度图像分别应用两个方向的 Sobel 算子，我们就得到了每个位置在 $x$ 方向和 $y$ 方向上的近似偏导，即**梯度**： $I_x$ 和 $I_y$ 。

这里还有一个细节：在图像的边界，卷积核会“伸出图像外”。因此，需要对这一圈数据进行边界处理，如抛弃、零填充、边缘复制或镜像填充，**镜像填充是实际工程中最常见的。**
## 2.2 窗口变化函数

我们已经得到了图像中每个位置的梯度 $I_x$ 和 $I_y$，下一步问题是：
...
![image.png](https://raw.githubusercontent.com/Goblinsscholar/Picture-bed/img/img/20260524125830356.png)
...
\end{bmatrix}  
$$
这个矩阵就叫： **Harris 矩阵**，它是整个算法的核心：（结构张量）
...
![42dc8483-aed3-439b-9187-6814b539e346.png](https://raw.githubusercontent.com/Goblinsscholar/Picture-bed/img/img/20260525213325882.png)
